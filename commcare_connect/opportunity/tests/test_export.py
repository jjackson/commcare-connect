...
def test_export_user_visit_data(mobile_user_with_connect_link):
    ...
    assert exporter.export("csv") == (
        ...
        "Entity ID,Entity Name,form.name,form.group.q\r\n"
...
def test_export_deliver_status_data(opportunity: Opportunity):
    ...
    for mobile_user in sorted(mobile_users, key=lambda x: x.name):
        ...
        user_visits = UserVisitFactory.create_batch(
            20, opportunity=opportunity, user=mobile_user, opportunity_access=access
        )
