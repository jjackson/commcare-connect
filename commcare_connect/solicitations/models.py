...
class Solicitation(BaseModel):
    ...
    description = models.TextField(
        help_text="Rich text description of the program and requirements"
    )
    target_population = models.CharField(
        max_length=255,
        help_text="Who will be served (e.g., 'Children under 5 in rural areas')"
    )
    scope_of_work = models.TextField(
        help_text="What work needs to be done"
    )
    solicitation_type = models.CharField(
        max_length=3,
        choices=SolicitationType.choices,
        default=SolicitationType.EOI
    )
    status = models.CharField(
        max_length=10,
        choices=SolicitationStatus.choices,
        default=SolicitationStatus.DRAFT
    )
    ...
    estimated_scale = models.CharField(
        max_length=255,
        blank=True,
        help_text="e.g., '40,000 children, 25-50 FLWs'"
    )
    
    # File attachments
    attachments = models.FileField(
        upload_to='solicitations/attachments/',
        null=True,
        blank=True,
...
class SolicitationQuestion(models.Model):
    ...
    question_type = models.CharField(
        ...
        default=QuestionType.TEXTAREA
    )
    is_required = models.BooleanField(default=True)
    options = models.JSONField(
        null=True,
        blank=True,
        help_text="For multiple choice questions, store options as JSON array"
    )
...
class SolicitationResponse(BaseModel):
    ...
    responses = models.JSONField(
        default=dict,
        help_text="Flexible storage for question/answer pairs"
    )
    status = models.CharField(
        max_length=20,
    ...
    attachments = models.FileField(
        upload_to='solicitations/responses/',
        null=True,
        blank=True
    )
...
class ResponseAttachment(BaseModel):
    ...
    file = models.FileField(
        upload_to='solicitations/response_attachments/',
