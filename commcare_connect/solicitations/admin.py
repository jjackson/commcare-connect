...
class SolicitationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': ('title', 'solicitation_type', 'program', 'created_by')
        }),
        ('Content', {
            'fields': ('description', 'target_population', 'scope_of_work', 'estimated_scale')
        }),
        ('Timeline', {
            'fields': ('expected_start_date', 'expected_end_date', 'application_deadline')
        }),
        ('Status', {
            'fields': ('status', 'is_publicly_listed')
        }),
        ('Files', {
            'fields': ('attachments',)
        }),
...
class SolicitationResponseAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': ('solicitation', 'organization', 'submitted_by', 'submission_date')
        }),
        ('Status', {
            'fields': ('status',)
        }),
        ...
        ('Progression', {
            'fields': ('progressed_to_solicitation',)
        }),
    )
