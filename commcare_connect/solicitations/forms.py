...
from .models import SolicitationResponse, SolicitationQuestion
        ...
        for question in questions:
            ...
            if question.question_type == 'text':
                field = forms.CharField(
                    ...
                    max_length=500,
                    widget=forms.TextInput(attrs={'class': 'w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-indigo focus:border-brand-indigo'})
                )
            elif question.question_type == 'textarea':
                field = forms.CharField(
                    label=question.question_text,
                    required=field_required,
                    widget=forms.Textarea(attrs={'class': 'w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-indigo focus:border-brand-indigo', 'rows': 4})
                )
            elif question.question_type == 'number':
                field = forms.IntegerField(
                    label=question.question_text,
                    required=field_required,
                    widget=forms.NumberInput(attrs={'class': 'w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-indigo focus:border-brand-indigo'})
                )
                ...
                field = forms.FileField(
                    label=question.question_text,
                    required=False,
                    widget=forms.FileInput(attrs={'class': 'w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-indigo focus:border-brand-indigo'})
                )
                ...
                field = forms.ChoiceField(
                    ...
                    required=field_required,
                    choices=choices,
                    widget=forms.Select(attrs={'class': 'w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-indigo focus:border-brand-indigo'})
                )
            else:
    ...
    def clean(self):
        ...
        if not self.user.memberships.exists():
            raise ValidationError(
                _("You must be a member of an organization to submit responses.")
            )
