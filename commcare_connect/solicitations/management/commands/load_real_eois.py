...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--update",
            action="store_true", 
            help="Update existing solicitations if they already exist"
        )
    ...
    def create_solicitations(self, solicitations_data, programs, dry_run=False, update=False):
        ...
        for sol_data in solicitations_data:
            ...
            if not program and not dry_run:
                self.stdout.write(
                    self.style.WARNING(f"Program not found: {sol_data['program_slug']}")
                )
                continue
            ...
            existing = Solicitation.objects.filter(
    ...
    def handle(self, *args, **options):
        ...
        if not dry_run:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully loaded real EOI data! "
