...
class RecordExplorerDataAccess:
    ...
    def get_all_records(
        ...
        """
        try:
            # If no experiment/type specified, get all by making multiple calls
            if not experiment or not type:
    ...
    def _get_all_records_all_types(
        ...
        Since the API requires experiment and type, we need to query
        for common experiment/type combinations.
        ...
        # Known experiment/type combinations
        known_combinations = [
            ("audit", "AuditTemplate"),
            ("audit", "AuditSession"),
            ("tasks", "Task"),
        ...
        for exp, rec_type in known_combinations:
            try:
                ...
                logger.warning(f"Failed to get records for {exp}/{rec_type}: {e}")
                continue
