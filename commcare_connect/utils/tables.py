...
class SelectableColumnTable(tables.Table):
    def __init__(self, *args, select_column_name="select", select_accessor="pk", **kwargs):
        self.base_columns[select_column_name] = tables.CheckBoxColumn(
            accessor=select_accessor,
            orderable=False,
            attrs={
                "th__input": {"@click": "toggleSelectAll()", "x-model": "selectAll", "name": "select_all"},
                "td__input": {"x-model": "selected", "@click.stop": "", "name": "row_select"},
            },
        )
        ...
        if current_sequence and current_sequence[0] == select_column_name:
            return

        if select_column_name in current_sequence:
            current_sequence.remove(select_column_name)

        current_sequence.insert(0, select_column_name)
        self.sequence = tuple(current_sequence)
        ...
        alpine_attrs = {
            ...
            "x-data": """{
                ...
                toggleSelectAll() {
                    ...
                    if (this.selectAll) {
                        this.selected = Array.from(document.querySelectorAll('input[type=checkbox][x-model=selected]'))
                            .map(cb => cb.value);
                    } else {
                        this.selected = [];
                    }
        ...
        self.attrs = {**self.attrs, **alpine_attrs}

    def inject_props_into_attr(self, existing_attr, prop):
        attr = self.attrs.get(existing_attr, "").strip()
        inner = attr[1:-1].strip()
        inner = f"{prop}, {inner}"
        self.attrs["x-data"] = f"{{ {inner} }}"
