...
class AuditDefinitionTest(TestCase):
    ...
    def test_mark_used(self):
        """Test marking an AuditDefinition as used"""
        audit_def = AuditDefinition.objects.create(
            opportunity_ids=[1, 2],
            audit_type="last_n_per_flw",
            granularity="per_flw",
        ...
        assert audit_def.status == AuditDefinition.Status.READY
        assert audit_def.sessions_created == 0
        assert audit_def.used_at is None

        audit_def.mark_used(sessions_created=2, username="test_auditor")

        assert audit_def.status == AuditDefinition.Status.USED
        assert audit_def.sessions_created == 2
        assert audit_def.used_by == "test_auditor"
        assert audit_def.used_at is not None
    ...
    def test_multiple_sessions_per_definition(self):
        ...
        assert audit_def.audit_sessions.count() == 3
        for session in sessions:
            assert session.audit_definition == audit_def

        # Mark as used
        audit_def.mark_used(sessions_created=3, username="auditor")
        assert audit_def.sessions_created == 3
    ...
    def test_preview_cleans_up_unused_definitions(self):
        """Test that creating a new preview cleans up unused definitions from the same user"""
        ...
        # Create some existing unused audit definitions for this user
        old_def1 = AuditDefinition.objects.create(
            ...
            granularity="per_flw",
            count_per_flw=5,
            status=AuditDefinition.Status.DRAFT,
        )
        ...
        # Create a used definition that should NOT be deleted
        used_def = AuditDefinition.objects.create(
        ...
        # Create a new preview - this should clean up old unused definitions
        ...
        # Verify old unused definitions were deleted
        ...
        # Verify used definition was preserved
        assert AuditDefinition.objects.filter(id=used_def.id).exists()

        # Verify other user's definition was preserved
        assert AuditDefinition.objects.filter(id=other_def.id).exists()

        # Verify new definition was created
        assert AuditDefinition.objects.count() == 3  # used_def, other_def, new_def
        new_def = result.audit_definition
        assert new_def.created_by == user
        assert new_def.status == AuditDefinition.Status.READY
