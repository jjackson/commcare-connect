...
def build_followup_from_pipeline(
    ...
    for form_dict in registration_forms:
        ...
        for sched in schedules:
            ...
            if form_username:
                mother_to_username[mother_case_id] = form_username
    ...
    # (pipeline completion rows have row.username + mother_case_id)
    for row in pipeline_rows:
        if row.username not in active_usernames:
            continue
        mother_case_id = row.computed.get("mother_case_id", "")
        if mother_case_id and mother_case_id not in mother_to_username:
            mother_to_username[mother_case_id] = row.username
    ...
    logger.info(
        "[MBW Follow-Up] Result: %d FLWs, %d total visits",
        len(by_flw),
...
def count_mothers_from_pipeline(
    ...
    if registration_forms:
        ...
        for row in pipeline_rows:
            ...
            mother_case_id = row.computed.get("mother_case_id", "")
            if mother_case_id:
                mothers_by_flw[row.username].add(mother_case_id)
